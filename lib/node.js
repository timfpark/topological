...
class Node {
    ...
    enqueueOutputMessages(messages, callback) {
        this.enqueueOutputMessagesTo(null, messages, callback);
    }

    enqueueOutputMessagesTo(connectionIds, messages, callback) {
            ...
            retryCallback => {
                this.processor.process(message, err => {
                    if (!err) {
                        ...
                        retriesLeft -= 1;
                        backoff *= 2;
                        console.log(`${this.id}: process failed with: ${err}`);
                ...
                console.log(`${this.id}: ${outcome} processing message: ${this.outstandingMessages} still in progress.`);
            }
        );
    }
    ...
    startMessageStreaming(input, callback) {
        input.stream((err, message) => {
            this.outstandingMessages += 1;
            console.log(`${this.id}: STARTING processing message: ${this.outstandingMessages} now in progress.`);
