...
class Node {
    ...
    complete(message, success) {
        ...
        if (success) {
            message.input.succeeded(message, err => {
                if (err)
                    this.log.error(`failed to mark message succeeded: ${err}`);
            });
        } else {
            message.input.failed(message, err => {
                if (err)
                    this.log.error(`failed to mark message failed: ${err}`);
            });
        }
    ...
    dispatch() {
        if (
            this.messagesProcessing < this.parallelism &&
            this.prefetchBuffer.length > 0
        ) {
        ...
        this.log.debug(
            `dispatch: ${this.messagesProcessing} / ${
                this.parallelism
            } processing, ${this.prefetchBuffer.length} buffered.`
        );
        ...
        if (this.paused && this.prefetchBuffer.length < this.prefetchCount) {
            ...
            async.each(
                ...
                err => {
                    if (err)
                        return this.log.error(`error resuming input: ${err}`);
                }
            );
        }
    }
    ...
    buffer(message) {
        ...
        if (
            !this.paused &&
            this.parallelism &&
            this.prefetchBuffer.length > this.prefetchCount
        ) {
            ...
            async.each(
                ...
                err => {
                    if (err)
                        return this.log.error(`error pausing input: ${err}`);
                }
            );
        }
            ...
            retryCallback => {
                this.processor.process(message, err => {
                    if (!err) {
                        ...
                        this.log.warn(
                            `retrying, attempts left: ${retriesLeft}, backing off: ${backoff}`
                        );
    ...
    startStreaming(callback) {
        async.each(
            ...
            (input, inputCallback) => {
                this.log.info(
                    `starting message processing for input ${input.id}`
                );

                input.stream((err, message) => {
                    if (err)
                        return this.log.warn(`stream reported error: ${err}`);
                    if (!message)
                        return this.log.warn(`null message received by node.`);
                    ...
                    async.each(
                        ...
                        (input, inputCallback) => {
                            this.log.info(
                                `starting input: ${
                                    input.id
                                } with config ${JSON.stringify(input.config)}`
                            );
                    ...
                    async.each(
                        ...
                        (output, outputCallback) => {
                            this.log.info(
                                `starting output: ${
                                    output.id
                                } with config ${JSON.stringify(output.config)}`
                            );
